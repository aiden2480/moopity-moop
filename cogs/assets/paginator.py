...
class Pages:
    ...
    async def numbered_page(self):
        ...
        try: await self.channel.delete_messages(to_delete)
    ...
    async def paginate(self, msg=None):
        ...
        while self.paginating:
            ...
            except: pass # can't remove it so don't bother doing so
...
class HelpPaginator(Pages):
    ...
    async def from_command(cls, ctx, command):
        ...
        self = cls(ctx, entries)
        self.title = f"{ctx.clean_prefix}{command.qualified_name} {command.signature}"
